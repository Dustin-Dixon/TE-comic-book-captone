<template>
  <v-container>
    ...
    <template v-if="hasCollection">
      <h6 class="text-nav">Total Comics In Collection: {{ stats.comicCount }}</h6>
      <v-divider class="my-1"></v-divider>
      <h3 class="text-center text-h6">Top 5 Characters</h3>
      <div v-for="characterCount in topFiveCharacters" :key="characterCount.character.id">
        <h6 class="text-nav">{{characterCount.character.name}}: {{characterCount.count}}</h6>
      </div>
      <v-divider class="my-1"></v-divider>
      <h3 class="text-center text-h6">Top 5 Creators</h3>
      <div v-for="creatorCount in topFiveCreators" :key="creatorCount.creator.id">
        <h6 class="text-nav">{{creatorCount.creator.name}}: {{creatorCount.count}}</h6>
      </div>
    </template>
  </v-container>
</template>
